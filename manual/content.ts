...
There are two versions of the UVP6:? The UVP6LP (Low Power) recommended for most application except CTD and fast AUVs? The UVP6HF (High Frequency), recommended for CTD and fast AUVs
2.2 Communication
...
Examples of computing the mean power in Watt for frequency under 0.8Hz.? typical acquisition at 0.1 Hz saving vignettes: (0.8 x 0.75 + 0.02 x (10-0.75))/10 = 0.08 W? typical acquisition at 0.2 Hz saving vignettes: (0.8 x 0.75 + 0.02 x (5-0.75))/5 = 0.14 W? typical acquisition at 0.5 Hz saving vignettes: (0.8 x 0.75 + 0.02 x (2-0.75))/2 = 0.32 W
...
3.1 At cruise level? Instrument mounting in its frame or on its hosting platform? Instrument connection to Power (AC or battery) and UVPapp
...
tween profiles (see below).? Charge battery (if applicable)? Time synchronisation? Porthole and light cleaning? Deployment (power ON or power piloted by the hosting vector)? Recovery (power OFF or power piloted by the hosting vector)? Rinsing with fresh water
3.3 On a daily / regular basis? Measure the voltage of the battery (if applicable)? Data download? Merge sequences (if applicable)? Sample creation (metadata filling)? Sample process and data Quality Control? Data backup? Delete data in instrument (if necessary)? Charge battery (if applicable)
3.4 End of cruise? Instrument removing from host vector or frame (never disassemble camera from arm)? Instrument cleaning and packing
...
ing the metadata first.? Download data? Merge sequences (if necessary)? Fill in sample metadata? Process data? Process images? Export to ODV (optional) and import into ODV? View vignettes (optional)? Load on the EcoPART FTP and import in EcoPART and EcoTAXA for image classification
...
the firewall.
The Ethernet link of the UVP6HF is sensitive:? Adding an Ethernet switch between the PC and the UVP6 data cable does facilitate the 
Ethernet communication of UVP6HF when cabled with the Y battery cable.? See the UVPapp IP address configuration chapter to set your PC? In case of Ethernet communication issue, it is suggested to try to set the Ethernet as below to 
...
dedicated sequences tools.
 With these tools it is possible to:? Create samples from the sequences: double click on a sequence to open the create-sample 
tool? ZIP vignettes if not done during download (essential for images process)? Erase sequence (quasi empty sequences are often created when an instrument set in AUTO 
mode is powered ON before being connected to UVPapp for data download)? Merge sequences
...
cated samples tools.
With these tools it is possible to:? Process the data and images of one or all project samples, generating the files in the ECO-
DATA folder of the projects. These files will later be imported in EcoPART and in EcoTAXA for 
image classification? Show the vignettes of the sample? Make profile plots? Export to ODV format
...
in different sections permits to:? Fill in SAMPLE METADATA: the operator will carefully fill in the different fields taking care of 
the sample naming to allow an easy later management of the resulting files? Select options for DATA SELECTION: the operator will define the type of dataset between 
...
a pressure sensor.? Check INSTRUMENT METADATA? Select the DATA RANGE: the data range will be manually defined with the help of the graph-
...
UVPapp will create the project folder which contains the following subfolders as a minimum:? calibration: users can store here the instrument calibration reports? config: contains the acquisition and hardware settings downloaded from the sensor and the 
archives of the previous programming. This folder is checked at every connection to the sen-
sor to check the consistency with the sensor internal settings.? CTDdata: a folder where users can place the data downloaded from the CTD which may be 
associated with the UVP6 in the same frame. The filename of the CTD data can be docu-
mented in the sample metadata form.? doc: users can store any document related to the project (cruise plans, scans...)? ecodata: the folder will contain the particle and image archives of each sample to be im-
ported in EcoTAXA? log: the sample may contain logs of the application tasks? meta: contains the table of metadata of the samples? raw: the folder contains the sequences downloaded from the instrument or, later, merged.? results: the folder contains graphs and tables produced by the data processing
...
7 SPECIFICATIONS
7.1 UVP6-LP? Operational depth: 0 to 6000 metres? Weight in air: 3.2 Kg? Weight in water: 1.5 Kg? Materials: titanium, glass and POM? Input voltage: 8-28 Vdc (0.1Watt@0.1Hz - 0.35Watt@0.5Hz - 1Watt@1.3Hz)? Interface: RS232, Ethernet, analog output, I/O? Lighting: 635nm, 50-400µS flashs? Resolution: 5Mpixels / 0.73µM? Field of view: 180 x 151mm x 23mm (0.6 L)? Max image frequency: 1.3Hz? Real time processing of images? Memory: > 400Go (1 TB option)? Pressure sensor (option): 0.1%FS accuracy 
7.2 UVP6-HF? Operational depth: 0 to 6000 metres? Weight in air: 3.2 Kg? Weight in water: 1.5 Kg? Materials: titanium, glass, POM and Aluminium (arm)? Input voltage: 10-28 Vdc (6 W / 0.02 W, acquisition / standby)? Interface: RS232, Ethernet, analog output, I/O? Lighting: 635nm, 50-100µS flashs? Resolution: 5Mpixels / 0.73µM? Field of view: 180 x 151mm x 23mm (0.6 L)? Max image frequency: 25Hz? Real time processing of images? Memory: > 400Go (1 TB option)? Pressure sensor: 0.1%FS accuracy
7.3 Standard battery (HYDRPTIC)? Operational depth: 0 to 6000 metres? Weight in air: 1.4 Kg? Weight in water: 0.6 Kg? Materials: titanium? Power: 75 Wh? Voltage: 21.6 Vdc
8 ANNEXE
8.1 Regulations
8.1.1 HS code
...
sequence of commands for:? Pre deployment checks? Acquisition commands? Other commands
...
It is important to consider the booting time of the instruments which impacts the responding time:? The UVP6LP boots in few ms => immediate response to messages? The UVP6HF boots in 14 seconds => it may respond after 14seconds
Procedure:? The sensor is powered ON? Acquisition starts with a START command, including the name of the setting table? The sensor returns HW and ACQ data lines (TAXO line if activated)? The sensor returns DATA lines
o On demand by "PT" commands if PILOTED
o At its own frequency if CONTINUOUS? The acquisition is stopped
o By powering OFF (file safe system)
o By sending a STOP command (preferable)? The sensor is turned OFF (if necessary)
...
HW_CONF,000002LP,0,UNDEFINED,0,000002VE,2,150,250,169559,20.000,393819,2000,2,192.168.0.128,0,356,6,23,1130.000,1.310,73,0.630,20190207,201907241218,marc.picheral@obs-
vlfr.fr,40.3,50.8,64,80.6,102,128,161,203,256,323,406,512,645,813,1020,1290,1630,2050
...
8.7.1 Shipping cardbox:? UVP6LP with light cable, dummy and optics covers
8.7.2 PELICASE? UVP6HF with light cables and optics covers? Octopus cable for serial, ethernet and power inlet? Serial to Usb converter? Ethernet Switch? Grease pot? USB key
Optional? Battery? Battery charger? Y cable for battery? Analog cable? CTD mounting kit
1
...
2
Overexposed frame have "0" for the number of images used and for all data
TAXO data frames:,8,17181,59,8,4778,41
...
It is recommended that you read all information buttons in the page if you are not familiar with EcoPart.? On the Ecotaxa home page, click on "Action" button > "Particle projects management" tool > "Create a new Particle project" green button? Name your project (Particle Project title) by coying the name of the Project to be imported. Select the root folder of your project from the FTP or the local UVP drives. The project name should start by "UVP6" according to the model of your instrument.? The metadata from the "cruise_info.txt" file of the project will be automatically filled when your press "Read metadata":
If the values are not correct or documented, you can edit the Project metadata in UVPapp, re- upload the header file from the meta folder into the FTP and then click on "Read Metadata" button to update.? The standard procedure is to use the "Create Ecotaxa project" tool to create a new project in which you will later import the images from the project. The alternative option is to link an existing Ecotaxa project if you already loaded the images (not recommended).? Set carefully the project owner from the list of Ecotaxa registered users. This people will be the only person allowed to later access to the project management page.? Change the different delays if necessary? Click on the "Save" button
